...
const weddingDate = new Date("2026-04-26T11:38:00+08:00");
...
if (form) {
  form.addEventListener("submit", async function (e) {
    ...
    try {
      ...
      resultDiv.innerHTML = "<p style='color:green;'>已收到您的回复 ❤️</p>";
      form.reset();
    } catch (err) {
      console.error(err);
...
// ===============================
// 微信分享卡片
// Cloudflare Worker 提供签名
// ===============================
(function initWeChatShare() {
  const ua = navigator.userAgent || "";
  const isWeChat = /micromessenger/i.test(ua);

  if (!isWeChat || typeof wx === "undefined") return;

  const currentUrl = window.location.href.split("#")[0];

  const shareData = {
    title: "MM和LL的婚礼邀请函",
    desc: "诚邀您参加MM和LL的婚礼，见证我们的幸福时刻。",
    link: currentUrl,
    imgUrl: "https://你的域名/images/share-cover.jpg"
  };

  // 改成你的 Cloudflare Worker 地址
  const signApi = "https://你的-worker地址.workers.dev/wx-sign";

  fetch(`${signApi}?url=${encodeURIComponent(currentUrl)}`)
    .then(res => res.json())
    .then(cfg => {
      if (!cfg.appId || !cfg.timestamp || !cfg.nonceStr || !cfg.signature) {
        throw new Error("微信签名返回不完整");
      }
